...
L'application suit une **architecture en couches professionnelle** avec séparation claire des responsabilités, gestion robuste des erreurs, et sécurité optimale.
...
- Permet aux clients de découvrir, réserver et gérer leurs tickets
...
#### Pour les Clients
- 📋 **Liste d'événements** triée par date
...
- 🔒 Saisie sécurisée des informations de paiement
...
```
┌─────────────────────────────────────┐
│  Rechercher & Filtrer Événements   │
│  ✓ Par type  ✓ Par lieu            │
├─────────────────────────────────────┤
│  📋 Liste des Événements           │
│  • Concert Coldplay - 15/12/2025   │
│  • Spectacle Cirque - 20/12/2025   │
├─────────────────────────────────────┤
│  🎫 Réserver Tickets               │
...
├─────────────────────────────────────┤
│  📊 Mes Événements & Statistiques   │
│  • Événement 1: 450€ | 85% rempli │
│  • Événement 2: 780€ | 92% rempli │
├─────────────────────────────────────┤
...
### 1. Cloner le Projet
```bash
git clone https://github.com/votre-username/plateforme-reservations.git
cd plateforme-reservations
```
...
mysql -u root -p plateforme_tickets < database/data.sql
```

### 3. Configurer la Connexion BD
Éditer `src/main/resources/config/database.properties` :
```properties
db.url=jdbc:mysql://localhost:3306/plateforme_tickets
db.username=root
...
- Chaque mot de passe unique (salt généré)
...
- **Architecture_JavaFX.pdf** : Guide d'architecture complet
- **Conception_BDD_Tickets.pdf** : Modèle de données
- **Requetes_SQL_Utiles.md** : 50+ requêtes SQL
- **Setup_IntelliJ_Complet.md** : Installation détaillée
- **Quick_Start_Templates.md** : Templates de code
...
┌──────────────────────────────────┐
│ Rechercher & Filtrer             │
│ [Tous les types ▼] [Tous lieux] │
├──────────────────────────────────┤
│ ÉVÉNEMENTS DISPONIBLES           │
│ • Coldplay - 15/12 - Accor Arena │
│   Standard: 65€ (500 dispo)      │
│   [Réserver]                     │
│                                  │
│ • Cirque du Soleil - 20/12 - ...│
│   VIP: 120€ (200 dispo)          │
│   [Réserver]                     │
└──────────────────────────────────┘
```
...
**Votre Nom**  
Développeur Full-Stack Java/JavaFX  
...
Merci à :
- **BNP Paribas** pour l'apprentissage en environnement professionnel
- **La communauté Java** pour les ressources et support
- **Les testeurs** pour leurs retours et suggestions
