...
public class EvenementDAO extends BaseDAO<Evenement> {
    ...
    public Evenement creer(Evenement evenement) throws DatabaseException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            pstmt.setObject(1, evenement.getOrganisateur());
            pstmt.setString(2, evenement.getTitre());
            pstmt.setDate(3, Date.valueOf(evenement.getDateEvenement()));
            ...
            if (affectedRows > 0) {
                try (ResultSet rs = pstmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        ...
                        logger.info("✓ Événement créé: {}", evenement.getTitre());
                        return evenement;
                    }
                }
            }
    ...
    public void mettreAJour(Evenement evenement) throws DatabaseException {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            ...
            LocalDate dateEvent = evenement.getDateEvenement();
            pstmt.setDate(2, Date.valueOf(dateEvent));
    ...
    private Evenement mapRowToEvenement(ResultSet rs) throws SQLException {
        ...
        evenement.setDateEvenement(rs.getDate("date_evenement").toLocalDate());
